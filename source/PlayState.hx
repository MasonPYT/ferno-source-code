...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'ignite':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ignite/dialog'));
			case 'ember':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ember/dialog'));
		}
		...
		switch(stageCheck)
		{
			...
			case 'hell':
			{
					defaultCamZoom = 0.9;
					curStage = 'hell';
					var bg:FlxSprite = new FlxSprite(-200, -100).loadGraphic(Paths.image('hell/lowkeythedevilvibespot'));
					bg.antialiasing = true;
			...
			case 'koolaidhell':
			{
					defaultCamZoom = 0.9;
					curStage = 'stage';
					var bg:FlxSprite = new FlxSprite(-200, -100).loadGraphic(Paths.image('koolaidhell/themanspiltthekoolaid'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
	
					var stageFront:FlxSprite = new FlxSprite(-650, 300).loadGraphic(Paths.image('koolaidhell/fire'));
		...
		switch (curStage)
		{
			...
			case 'ignite':
				...
				{
				// trailArea.scrollFactor.set();
				var frenoTrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
				// frenoTrail.changeValuesEnabled(false, false, false, false);
				// frenoTrail.changeGraphic()
				add(frenoTrail);
				// frenoTrail.scrollFactor.set(1.1, 1.1);
				}
